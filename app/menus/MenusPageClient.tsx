...
function getProductsForSubsection(subsectionId: string, allProducts: CateringProduct[]): CateringProduct[] {
  ...
  if (!filter) return [];
  return sortByClassification(allProducts.filter(filter));
}
...
export default function MenusPageClient({ products }: { products: CateringProduct[] }) {
  ...
  return (
    <div className="min-h-screen bg-pepe-cream">
      ...
      <div className="container mx-auto px-4 py-8 sm:py-12">
        ...
        <HeroItemsSection products={products} mode="browse" />
          ...
          >
            ...
            <div className="space-y-12">
              {section.subsections.map((subsection) => {
                const subsectionProducts = getProductsForSubsection(subsection.id, products);
                if (subsectionProducts.length === 0) return null;
