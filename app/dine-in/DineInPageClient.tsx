...
import { DineInMenuSection } from '@/lib/dine-in-menu';
...
interface DineInPageClientProps {
  menu: DineInMenuSection[];
  foodSections: string[];
  drinkSections: string[];
  promos: { title: string; description: string }[];
}

export default function DineInPageClient({ menu, foodSections, drinkSections, promos }: DineInPageClientProps) {
  const [activeSection, setActiveSection] = useState<string>('appetizers');
  const [activeTab, setActiveTab] = useState<'food' | 'drinks'>('food');

  const currentSections = activeTab === 'food'
    ? menu.filter(s => foodSections.includes(s.id))
    : menu.filter(s => drinkSections.includes(s.id));
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex justify-center py-3 gap-2">
            <button
              onClick={() => {
                setActiveTab('food');
                setActiveSection(foodSections[0]);
              }}
            ...
            <button
              onClick={() => {
                setActiveTab('drinks');
                setActiveSection(drinkSections[0]);
              }}
              className={`px-6 py-2 rounded-full font-oswald text-sm sm:text-base tracking-wider transition-all ${
                activeTab === 'drinks'
                  ? 'bg-pepe-burnt-orange text-white'
